...
static void GPIO_FSMC_Configuration(void);
static void DM9000A_FSMCConfig(void);
static rt_uint8_t regread(rt_uint16_t reg);
static void regwrite(rt_uint16_t reg, rt_uint16_t value);
...
rt_uint32_t dm9000x_read_id(void)
{
    ...
    {
        //dm9000x_inital();
        id  = regread( DM9000_VIDL ) &0x00ff;
        id |= (regread( DM9000_VIDH )&0x00ff) << 8;
        id |= (regread( DM9000_PIDL )&0x00ff) << 16;
        id |= (regread( DM9000_PIDH )&0x00ff) << 24;
...
void dm9000x_set_macaddr(rt_uint8_t *macaddr)
{
    ...
    for(i=0; i<6; i++)
        regwrite(DM9000_PAR + i, macaddr[i]);

    //printf("Mac: ");
    //for(i=0; i<6; i++)
    //    printf("%x:",regread(DM9000_PAR+i));
    //printf("\n");
}
...
void dm9000x_inital(rt_uint8_t *macaddr)
{
    ...
    regwrite(DM9000_GPCR, 0x01);

    /* GPR bit[0]=0 使DM9000的GPIO3输出为低以激活内部PHY */
    regwrite(DM9000_GPR, 0x00);

    /* 延时2ms以上等待PHY上电 */
    dm9000_delay(20000);

    /* 软件复位 */
    regwrite(DM9000_NCR, 0x03);

    /* 延时20us以上等待软件复位完成 */
    dm9000_delay(5000);

    /* 复位完成，设置正常工作模式 */
    regwrite(DM9000_NCR, 0x00);
    ...
    /*  */
    regwrite(DM9000_NSR, 0x2c);//清除各种状态标志位

    regwrite(DM9000_ISR, 0x3f);//清除所有中断标志位

    regwrite(DM9000_RCR, 0x39);//接收控制

    regwrite(DM9000_TCR, 0x00);//发送控制

    regwrite(DM9000_BPTR, 0x3f);

    regwrite(DM9000_FCTR, 0x3a);

    regwrite(DM9000_FCR, 0xff);

    regwrite(DM9000_SMCR, 0x00);
    ...
    LOG_D("Mac: ");
    for(i=0; i<6; i++)
        LOG_RAW("%x:",regread(DM9000_PAR+i) & 0x00ff);

    LOG_RAW("\n");


    regwrite(DM9000_NSR, 0x2c);

    regwrite(DM9000_ISR, 0x3f);

    regwrite(DM9000_IMR, 0x81);
}
void dm9000x_sendpacket( rt_uint8_t* packet, rt_uint16_t len)
{
    ...
    rt_uint16_t io_mode;
    rt_uint16_t i,Temp;
    regwrite(DM9000_IMR, 0x80);//先禁止网卡中断，防止在发送数据时被中断干扰

    io_mode = regread(DM9000_ISR) >> 7;

    if(DM9000X_BYTE_MODE == (io_mode & 0x01))
    {
        while(length--)
        {
            regwrite(DM9000_MWCMD,*(packet++));
        }
    }
    else
    {
        ...
        {
            //dm9000_delay(20);
            //regwrite(DM9000_MWCMD,(packet[i]&0xff));
            //regwrite(DM9000_MWCMD,(packet[i]>>8));
            Temp = packet[i]|( packet[i+1]<< 8);
            regwrite(DM9000_MWCMD,Temp);
        }
        LOG_D("dm9000x_sendpacket:dm9000 io mode is 16bit.....!\n");
    }
    regwrite(DM9000_TXPLH, (len>>8) & 0x0ff);
    regwrite(DM9000_TXPLL, len & 0x0ff);

    regwrite(DM9000_TCR, 0x01);//发送数据到以太网上

    while(!(regread(DM9000_NSR)&0x0C))
        dm9000_delay(5);

    regwrite(DM9000_NSR, 0x00);//清除状态寄存器，由于发送数据没有设置中断，因此不必处理中断标志位

    regwrite(DM9000_IMR, 0x81);//DM9000网卡的接收中断使能
}
...
rt_uint16_t dm9000x_receivepacket(rt_uint8_t* packet, rt_uint16_t maxlen)
{
    ...
    rt_uint16_t  rx_status = 0x00;
    rt_uint16_t  rx_length = 0x00;
...
/*以上为有效数据包前的4个状态字节*/
    if(regread(DM9000_ISR) & 0x01)
    {
        regwrite(DM9000_ISR, 0x01);
    }

/*清除接收中断标志位*/

    ready = regread(DM9000_MRCMDX); // 第一次读取，一般读取到的是 00H
    if((ready & 0x0ff) != 0x01)
    {
        ready = regread(DM9000_MRCMDX); // 第二次读取，总能获取到数据
        if((ready & 0x01) != 0x01)
        {
            if((ready & 0x01) != 0x00) //若第二次读取到的不是 01H 或 00H ，则表示没有初始化成功
            {
                 regwrite(DM9000_IMR, 0x80);//屏幕网卡中断
                 dm9000x_inital(mac_addr);//重新初始化
                 regwrite(DM9000_IMR, 0x81);//打开网卡中断
            }
            goto rx_erro;
         }
    }

    rx_io_mode = regread(DM9000_ISR) >> 7;

    if(DM9000X_BYTE_MODE == (rx_io_mode & 0x01))
    {
        rx_status = regread(DM9000_MRCMD) + (regread(DM9000_MRCMD) << 8);
        rx_length = regread(DM9000_MRCMD) + (regread(DM9000_MRCMD) << 8);
